...
│   ├── ItemServiceApplication.java     # 스프링 부트 메인 클래스
│   ├── TestDataInit.java               # 테스트용 초기 데이터 등록
│   ├── domain/                         # 도메인 모델 (Item, ItemRepository)
│   └── web/                            # 웹 계층 (Controller)
└── resources/
    ...
    └── templates/
        └── message/
            ├── items.html              # 상품 목록 (메시지 적용)
            ├── item.html               # 상품 상세 (메시지 적용)
            ├── addForm.html            # 상품 등록 폼 (메시지 적용)
            └── editForm.html           # 상품 수정 폼 (메시지 적용)
```
...
실행 후 브라우저에서 접속: [http://localhost:8080/message/items](http://localhost:8080/message/items)
